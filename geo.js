(function () {
        ...
        httpRequest.onreadystatechange = function () {
            if (httpRequest.readyState === 4) {
                if (httpRequest.status === 200) {
                    ...
                    // alterar a função abaixo para a IGRFv** a ser utilizada
                    ON_DAED['GEO'].IGRF = IGRFv14;
                }
            }
        };

        // alterar o arquivo fonte do igrf a ser utilizado
        httpRequest.open('GET', 'igrf-v14.json', false);
        httpRequest.send();
    ...
    function IGRFv12(isv, date, itype, alt, colat, elong) {
        ...
        if (yearData >= 1900 && yearData <= 2020) {
    ...
    // src: NOAA
    function IGRFv13(isv, date, itype, alt, colat, elong) {

        var yearData = parseInt(date);

        var ret = null;

        if (yearData >= 1900 && yearData <= 2030) {
    ...
    function IGRFv14(isv, date, itype, alt, colat, elong) {
        var yearData = parseInt(date);

        var ret = null;

        let dateCheck = 2025.0;

        if (yearData >= 1900 && yearData <= 2035) {
